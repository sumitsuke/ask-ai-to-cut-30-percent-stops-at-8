...
SRC = (
    sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "materials")
)  # 元＝サイトの src/content/guide（09-19 夜: 絶対パスを引数に。materials/ が既にあればそれを読む）
...
for f in sorted(glob.glob(os.path.join(SRC, "*.md"))):
    ...
    p = os.path.join(M, slug + ".md")
    io.open(p, "w", encoding="utf-8", newline="\n").write(body)
    h = hashlib.sha256(body.encode("utf-8")).hexdigest()
    sha.append(f"{h}  {slug}.md")
    plain = re.sub(r"<[^>]+>", "", body)
    atoms = {}
    for m in ATOM.finditer(plain):
        ...
        key = f"{val} {unit}"
        atoms.setdefault(key, []).append(ctx)
    claims = [c.strip() for c in CLAIM.findall(plain)] if False else [m.group(0).strip() for m in CLAIM.finditer(plain)]
    out[slug] = {
        "chars": len(text_only(body)),
        "atoms": {k: {"count": len(v), "contexts": v} for k, v in atoms.items()},
        "claim_candidates": claims,
    }
    print(
        slug, out[slug]["chars"], "atoms", sum(a["count"] for a in out[slug]["atoms"].values()), "claims", len(claims)
    )
io.open(os.path.join(HERE, "atoms_claims_candidates.json"), "w", encoding="utf-8").write(
