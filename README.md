...
`claude -p --model claude-opus-5` was asked, one document per call, to cut each of 14 texts (1,149–1,797 characters) by 20–30%. Four instruction conditions were nested: **0** no constraint at all; **A** keep the heading/paragraph/list structure; **B** A + keep numbers and sentences stating conditions, limits and exceptions; **C** B + lists may be converted to bullets. Median change: **0 −6.5% / A −6.9% / B −8.2% / C −8.4%**. Only **3 of 56** outputs reached −20%. The differences between conditions (≤1.3 pt) are inside the reproduction width measured beforehand (9 pilot runs, median width 6.6 pt), so the pre-registered hypotheses about the "keep" condition are HOLD; the main result is outside them — under these run conditions the requested 20–30% was not reached even with no constraints (what sets the ceiling was not identified). The three outputs that did reach −20% were all B or C, so the "keep" condition did not prevent compression either. "Number + unit" strings still disappeared (27/56 outputs lost at least one, almost always a count such as 「3 つ」; every price 「33,000 円」 survived, 18/18). This is a string check, not a meaning check — whether the count survived as a bullet list was not adjudicated. Boundary sentences: 84/87 kept for A–C after human adjudication of 33 paraphrases (3 lost, all the same sentence); condition 0: 28/29 (`claims_judged_cond0_2026-09-19.csv`). The one lost sentence is the **same sentence in all four conditions** — 「合否条件を通した検証の結果」 ("the results of verification that passed the acceptance criteria") shortened to 「検証の結果」 every time, i.e. the acceptance-criteria wording was dropped whether or not the instruction asked to keep it. (Its condition-0 row was first batch-judged as a paraphrase and then aligned with A–C after review; the change is recorded in the file.)
