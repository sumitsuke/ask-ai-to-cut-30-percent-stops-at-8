...
ATOM = re.compile(
    r"([\d,]+(?:\.\d+)?)\s*(円|件|本|日|時間|営業日|%|％|項目|段階|か所|回|人|社|通|枚|行|字|ミリ秒|秒|分|か月|つ|層|型|種|点|倍)"
)
...
def norm(s):
    return re.sub(r"[\s*_`「」（）()\[\]【】、。・:：―—-]", "", plain(s))


def atoms_of(s):
    d = {}
    for m in ATOM.finditer(plain(s)):
...
def structure(s):
    return (len(re.findall(r"^#{1,6} ", s, re.M)), len(re.findall(r"^\s*(?:[-*]|\d+\.) ", s, re.M)))
    ...
    rows.append(
        {
            ...
            "src_chars": tlen(src),
            "out_chars": tlen(out),
            "change_pct": round((tlen(out) - tlen(src)) / tlen(src) * 100, 1),
            "reached_-20": tlen(out) <= tlen(src) * 0.8,
            "atoms_total": sum(a0.values()),
            "atoms_lost": sum(lost.values()),
            "atoms_lost_list": ";".join(f"{k}×{v}" for k, v in lost.items()),
            "claims_total": len(F[slug]["claims"]),
            "claims_auto_pass": hit,
            "claims_to_judge": len(F[slug]["claims"]) - hit,
            "headings_src_out": f"{s0[0]}/{s1[0]}",
            "bullets_src_out": f"{s0[1]}/{s1[1]}",
        }
    )
...
with io.open(os.path.join(HERE, "claims_to_judge_cond0.csv"), "w", encoding="utf-8", newline="") as fh:
    w = csv.writer(fh)
    w.writerow(["slug", "cond", "claim", "裁定（言い換えて残った／落ちた／反転）", "根拠"])
    w.writerows([j for j in judge if j[1] == "0"])
lines = [
...
for cond in "0ABC":
    ...
    ch = sorted(r["change_pct"] for r in rs)
    n = len(ch)
    lines.append(
        f"| {cond} | {n} | {st.median(ch):.1f}% | {ch[n // 4]:.1f}〜{ch[(3 * n) // 4]:.1f} | {ch[0]:.1f}／{ch[-1]:.1f} | {sum(r['reached_-20'] for r in rs)} | {sum(r['atoms_lost'] for r in rs)}／{sum(1 for r in rs if r['atoms_lost'])} | {sum(r['claims_auto_pass'] for r in rs)}／{sum(r['claims_total'] for r in rs)} | {sum(r['claims_to_judge'] for r in rs)} |"
    )
