...
CWD = os.path.join(os.environ['TEMP'], 't34'); os.makedirs(CWD, exist_ok=True)
def text_only(s): return len(re.sub(r'\s', '', re.sub(r'<[^>]+>', '', s)))
def gen(slug, cond, rep, mode):
    body = io.open(os.path.join(M, slug + '.md'), encoding='utf-8').read()
    prompt = COND[cond] + '\n\n---\n\n' + body
    out_dir = os.path.join(HERE, 'outputs', mode); os.makedirs(out_dir, exist_ok=True)
    ...
    r = subprocess.run(['claude', '-p', '--model', MODEL], input=prompt, cwd=CWD, capture_output=True, text=True, encoding='utf-8', errors='replace', shell=True, timeout=600)
    txt = r.stdout.strip()
    io.open(out, 'w', encoding='utf-8', newline='\n').write(txt + '\n')
    with io.open(os.path.join(HERE, 'outputs', 'log.csv'), 'a', encoding='utf-8', newline='') as f:
        csv.writer(f).writerow([mode, slug, cond, rep, time.strftime('%Y-%m-%dT%H:%M:%S'), round(time.time() - t0), text_only(body), text_only(txt), r.returncode, (r.stderr or '')[:120].replace('\n', ' ')])
...
if mode == 'cond0':
    for slug in slugs: gen(slug, '0', 1, 'main')
elif mode == 'pilot':
    for slug in slugs[:3]:
        for rep in (1, 2, 3): gen(slug, 'A', rep, 'pilot')
else:
    rnd = random.Random(20260919); order = {}
    for slug in slugs:
        conds = ['A', 'B', 'C']; rnd.shuffle(conds); order[slug] = conds
    io.open(os.path.join(HERE, 'outputs', 'main_order.json'), 'w', encoding='utf-8').write(json.dumps({'seed': 20260919, 'order': order}, ensure_ascii=False, indent=1))
    for slug in slugs:
        for cond in order[slug]: gen(slug, cond, 1, 'main')
print('done')
